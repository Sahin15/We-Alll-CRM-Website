...
const HoDWorkLogReview = () => {
  ...
  const [quickFilter, setQuickFilter] = useState("yesterday");
  ...
  const fetchDepartmentWorkLogs = async () => {
    try {
      setLoading(true);
      const params = {
        ...filters,
  ...
  const handleReviewSubmit = async (reviewNotes, status) => {
    try {
      await workLogApi.reviewDepartmentWorkLog(
        selectedLog._id,
        reviewNotes,
        status
      );
      ...
      toast.error("Failed to review work log");
      console.error(error);
    }
  };
  ...
  return (
    <Container fluid className="mt-4">
      <Row className="mb-4">
        <Col>
          <h2>Department Work Log Review</h2>
...
export default HoDWorkLogReview;
